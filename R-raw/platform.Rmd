...
```{r}
# unique %>% yaml::write_yaml('output/temp.yaml', column.major = F)
library(tidyverse)
library(parallel)
devtools::load_all()
```
...
```{r}
gpls <- readr::read_rds('/media/computer/research/lem4/data-raw/gpls.rds')
n_all <- length(gpls)
...
detail <- function(x, info_only = F) {
	if (info_only)
		print(gpls[unique(x$accession)] %>% lapply(. %>% {.$info}))
	else
		print(gpls[unique(x$accession)])
}
```
...
```{r}
gpl_null <- gpls %>% {.[sapply(., . %>% {.$info} %>% is.null)]} %>% names %T>% {print(head(.))}

gpls %<>% {.[setdiff(names(.), gpl_null)]}
```
...
assertthat::assert_that(
	gpls %>% lapply(. %>% {colnames(.$sample) %>% {.[duplicated(.)]}}) %>%
...
```{r make-info}
info_all <- setdiff(names(gpls), special$accession) %>%
	mclapply(. %>% {add_column(gpls[[.]]$info, accession = ., .before = 1)}) %>%
...
> to do: compare which rule is better
...
```{r}
	...
	# lapply(. %>% {tibble(accession = ., symbol = select(gpls[[.]]$sample, contains('symbol'))[[1]])}) %>% bind_rows() %>% filter(symbol != '') %>% filter(!(symbol %in% hgnc::hugo_symbol_all)) %>% group_by(accession) %>% summarise(n = n()) %>% arrange(desc(n))
```
...
devtools::use_data(gpl, overwrite = T)
```
